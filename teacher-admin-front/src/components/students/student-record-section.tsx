...
function getFormFromRecord(record: StudentRecord): FormState {
  return {
    schoolYear: getRecordSchoolYear(record),
    semester: getRecordSemester(record),
    specialNote: record.specialNote ?? "",
    behaviorNote: record.behaviorNote ?? "",
    careerHope: record.careerHope ?? "",
    healthNote: record.healthNote ?? "",
  };
}

function getRecordSummary(record: StudentRecord) {
  if (record.updatedAt) return `최근 수정 ${formatDateTime(record.updatedAt)}`;
...
export default function StudentRecordSection({ studentId }: Props) {
  ...
  const fetchRecords = async () => {
    try {
      ...
      if (data.length > 0) {
        ...
        setForm(getFormFromRecord(latest));
      } else {
        setForm(emptyForm);
      }
    } catch (err) {
  ...
  const handleSave = async () => {
    try {
      ...
      const responseRecord = await saveStudentRecord(studentId, form);
      const savedRecord = {
        ...responseRecord,
        updatedAt: responseRecord.updatedAt ?? new Date().toISOString(),
      };
      const savedForm = getFormFromRecord(savedRecord);

      setRecords((prev) => {
        const exists = prev.some(
          (record) =>
            record.id === savedRecord.id ||
            isSameRecordPeriod(record, savedForm.schoolYear, savedForm.semester)
        );
